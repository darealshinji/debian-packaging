{ patchelfmodSrc ? { outPath = ./.; revCount = 1234; shortRev = "abcdef"; }
  ...
  jobs = rec {
    ...
    tarball =
      pkgs.releaseTools.sourceTarball rec {
        name = "patchelfmod-tarball";
        version = builtins.readFile ./version + (if officialRelease then "" else "pre${toString patchelfmodSrc.revCount}_${patchelfmodSrc.shortRev}");
    ...
    release = pkgs.releaseTools.aggregate
      { name = "patchelfmod-${tarball.version}";
        constituents =
          [ tarball
            build.x86_64-linux
            build.i686-linux
